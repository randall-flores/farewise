...
- **`serpapi` (default, the only REAL source).** SerpApi's Google Flights engine, called server-side only (`lib/serpapi.js`, `SERPAPI_KEY` is secret), normalized by `lib/normalize-serpapi.js`. `currency=USD`, `hl=en`, `gl=us`. Production default.
  ...
  - **Round-trip** (return date set): two calls, `type=1`, **`deep_search=true`** on both. Call 1 returns outbound options (each with a `departure_token` and the round-trip total). We take the **cheapest outbound** and call again with its `departure_token`; call 2's returns (in `other_flights`) are paired with that one outbound into full round trips (`normalizeRoundTrip`). So v1 round-trip shares one outbound and varies the return — a future improvement is multiple outbounds. The headline price is the **round-trip total**, labeled "round trip" on the card. The return option's `booking_token` covers **both legs** (verified: the booking call's `selected_flights` has 2 legs), so the seller link books the whole itinerary (booking call also uses `deep_search` for round trips).
    - **Why `deep_search`:** SerpApi's shallow default returns *empty* for some routes that Google Flights actually shows (e.g. SJO→BER). `deep_search=true` returns browser-identical inventory. Costs extra latency; required for correct round-trip results.
...
- **No fake fallback.** We never fall back to demo/fake data. Two distinct empty/failure states, kept apart:
  - **Genuine empty** (SerpApi succeeds but returns no flights for these dates — including its "hasn't returned any results" message): a **calm** "no flights found for these dates" empty-state (HTTP 200, `flights: []`). Not an error.
  - **Real failure** (network / HTTP / unexpected SerpApi error): the **red** on-brand error down-state (HTTP 502). We'd rather show nothing than prices we can't verify.
- **Booking/seller links are NOT fetched on the results page.** `bookVia.url` stays `null`; the SerpApi **`booking_token`** is kept in `bookVia.token`. Booking options are fetched **lazily** only when the user opens "How to book" on a card, via `POST /api/book-options` (a second SerpApi google_flights call with the token + the original search context). One-way results carry `booking_token`, **not** `departure_token` (that one is only for round-trip return legs — verified against the live response). Options are normalized by `normalizeBookingOptions` (honest unknowns: `airline === true` only = direct, missing `extensions`/`option_title` → `[]`/`null`, no `booking_request` → no book button). The redirect is a **POST** to Google's `clk/f` endpoint with `post_data` (the UI submits a real form, never an `<a href>`). Results cached per token ~30 min to save credits. Empty/failure → honest on-brand message, never a fabricated link.
