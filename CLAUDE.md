...
- **Booking/seller links are NOT fetched on the results page.** `bookVia.url` stays `null`; the SerpApi **`booking_token`** is kept in `bookVia.token` for a later lazy fetch (on card expand / book click). One-way results carry `booking_token`, **not** `departure_token` (that one is only for round-trip return legs — verified against the live response). The placeholder spot is marked in `search-experience.js` and `normalize-serpapi.js`.
