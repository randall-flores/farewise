...
// Duffel place suggestions -> simple { code, label, type, city } for the dropdown.
// Skips any place without an IATA code (we search by code, so a codeless place is useless).
export function normalizePlaces(places = []) {
  ...
  for (const p of places) {
    ...
    const label =
      p.type === "city"
        ? `${p.name} — all airports (${p.iata_code})`
        : `${p.name} (${p.iata_code})`;
    out.push({ code: p.iata_code, label, type: p.type, city: p.city_name || p.name });
  }
  return out;
}
