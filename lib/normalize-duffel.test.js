...
describe("normalizePlaces", () => {
  const raw = [
    { type: "city", name: "Miami", iata_code: "MIA", city_name: "Miami" },
    { type: "airport", name: "Miami International Airport", iata_code: "MIA", city_name: "Miami" },
    { type: "airport", name: "Fort Lauderdale Hollywood Intl", iata_code: "FLL", city_name: "Fort Lauderdale" },
    { type: "airport", name: "No Code Field" }, // no iata_code -> skipped
  ];

  it("skips places with no IATA code", () => {
    expect(normalizePlaces(raw)).toHaveLength(3);
  });

  it("labels a city as 'all airports' and an airport with its name + code", () => {
    const out = normalizePlaces(raw);
    expect(out[0]).toEqual({ code: "MIA", label: "Miami — all airports (MIA)", type: "city", city: "Miami" });
    expect(out[1]).toMatchObject({ code: "MIA", label: "Miami International Airport (MIA)", type: "airport" });
    expect(out[2].label).toBe("Fort Lauderdale Hollywood Intl (FLL)");
  });
});
