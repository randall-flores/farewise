...
// `onStage` is optional. When the caller passes one, it's called with the name of
// each phase as that phase actually begins, so a streaming caller can tell the
// user where the wait is going. It reports what happened; it never predicts.
export async function getFlights(search, onStage) {
  ...
  if (search.returnDate) {
    const { outbound, returnsJson, priceInsights } = await searchSerpApiRoundTrip(search, onStage);
    ...
    // An empty list here is a GENUINE "no flights for these dates" — the route
    // turns it into a calm empty-state, not the red error. A real API failure
    // throws upstream (serpGet) and becomes the error down-state instead.
  ...
  // Empty list = genuine "no flights" (calm empty-state). Real failures throw.
