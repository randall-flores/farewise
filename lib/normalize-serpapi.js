...
// One SerpApi flight item's `flights[]` -> our segment shape.
function mapSegments(rawSegs = []) {
  return rawSegs.map((f) => ({
...
export function normalizeSerpFlight(item, index, search = {}) {
  ...
  const segments = mapSegments(rawSegs);
...
// Round-trip: pair ONE outbound option (from call 1) with each return option
// (call 2). Every result is a full round trip — outbound + return segments, the
// round-trip TOTAL price, and a booking_token that covers both legs. Same honest
// unknowns as one-way (fees/baggage/refundable/protection not in search data).
export function normalizeRoundTrip(outbound, returnsJson = {}, search = {}) {
  if (!outbound) return [];

  const returns = [...(returnsJson.best_flights || []), ...(returnsJson.other_flights || [])].filter(
    (r) => typeof r.price === "number" && Number.isFinite(r.price)
  );

  const outSegs = mapSegments(outbound.flights || []);
  const outStops = Math.max(0, outSegs.length - 1);
  const cabin = search.cabin || "economy";

  return returns.map((ret, i) => {
    const retSegs = mapSegments(ret.flights || []);
    const retStops = Math.max(0, retSegs.length - 1);
    const price = Math.round(ret.price); // round-trip total for this outbound+return
    ...
    return {
      ...
      // Outbound stays in `segments` so existing one-way rendering still works.
      totalDuration: minutesToText(outbound.total_duration),
      stops: outStops,
      segments: outSegs,
      layover: outStops > 0 ? firstLayover(outbound) : "",

      // Return leg.
      returnTotalDuration: minutesToText(ret.total_duration),
      returnStops: retStops,
      returnSegments: retSegs,
      returnLayover: retStops > 0 ? firstLayover(ret) : "",

      // Unknown from search data — never invented.
      bookingType: null,
      protected: null,
      baggage: { carryOn: null, checked: null },
      extraFees: {},
      ...
      // booking_token from the RETURN option covers both legs (verified).
      bookVia: { name: outSegs[0]?.airline || "the airline", url: null, token: ret.booking_token || null },
    };
  });
}
