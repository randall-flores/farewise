...
export function normalizeSerpFlight(item, index, search = {}) {
  ...
  return {
    ...
    // Google Flights itineraries are single, ticketed bookings — not self-transfers.
    ...
    // url stays null; `token` (departure_token) is kept so a later lazy fetch —
    // on card expand / book click — can resolve real booking options.
    // TODO(phase: booking links): fetch booking options here with bookVia.token.
    bookVia: { name: airline, url: null, token: item.departure_token || null },
  };
}
