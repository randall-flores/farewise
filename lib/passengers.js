...
// Human label, e.g. "2 adults, 1 child", "1 adult, 2 babies". "Baby" rather than
// "infant" for the same reason the panel says it: it's the word people use.
// Babies with a seat and babies on a lap merge into one count for the label
// only; the panel still tracks them separately. Adults always show (min 1).
export function passengerSummary(counts) {
  ...
  const babies = (counts.infantsInSeat || 0) + (counts.infantsOnLap || 0);
  if (babies > 0) parts.push(count(babies, "baby", "babies"));
  return parts.join(", ");
}
