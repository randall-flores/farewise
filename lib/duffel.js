...
const PLACES_URL = "https://api.duffel.com/places/suggestions";
...
// Airport/city autocomplete. Same token, same Duffel account — no new API.
// Returns Duffel's raw place objects; normalize-duffel turns them into our shape.
export async function searchPlaces(query) {
  ...
  const url = `${PLACES_URL}?query=${encodeURIComponent(query)}`;
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Duffel places failed (${res.status}): ${text.slice(0, 200)}`);
  }
