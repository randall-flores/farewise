...
import { allInPrice, detectRisks, reconcileVerdict } from "./flight-helpers.js";
...
describe("allInPrice", () => {
  ...
  it("handles a missing extraFees key", () => {
    expect(allInPrice({ price: 300 })).toBe(300);
  });

  it("returns 0 for a missing flight", () => {
    expect(allInPrice(null)).toBe(0);
  });
});
...
describe("detectRisks — airport change", () => {
  ...
  it("does not flag airport-change on a non-stop flight", () => {
    const nonStop = {
      ...airportChangeFlight,
      stops: 0,
      segments: [{ from: "MIA", to: "BER", airline: "LH", flightNo: "LH1", duration: "10h" }],
    };
    expect(detectRisks(nonStop, [nonStop]).find((r) => r.type === "airport-change")).toBeFalsy();
  });
});
