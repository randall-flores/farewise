  ...
  input_schema: {
    ...
    properties: {
      summary: {
        type: "string",
        description:
          "A short headline-style verdict that POINTS, it does not re-explain — the cards below carry the detail. One tight line per flight, separated by newlines (\\n), about 3-4 lines total. Each line: airline name + price + the single most important point (e.g. best balance / easiest / high risk / price not yet confirmed). Transactional, plain language, no airline jargon. Do not write paragraphs and do not repeat the full reasoning. Example line: 'TAP — ~$467. Best balance: one ticket, real protection.'",
      },
      flights: {
        type: "array",
...
- summary: a short headline verdict that POINTS — one tight line per flight (newline-separated), about 3-4 lines total, each with airline + price + the single most important point. Do NOT re-explain the flights; the cards carry the detail.
...
- Write every tag, explanation, and summary in the FareWise voice: transactional (fact + why it matters, no mood words, no padding) and plain language (no airline jargon — never "layover", "leg", "nonstop", "self-transfer", "all-in"; use the plain swaps). Risk warnings are the exception: make them full, blunt, plain sentences, not shorthand.
